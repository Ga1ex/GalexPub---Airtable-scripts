const H_TABLE = base.getTable('Move to new VTC'); 
const V_TABLE = base.getTable('Title Chain test');
const UW=['Unit-Week','Unit','Week'];
const [SO,MS]=['Owner Info','Minus '];
...
const create=(rc)=>owners(rc).map((ow,ix)=>({fields:Object.
    fromEntries([...leftside(rc),[SO,ow],['seq',ix],['Link',[{id:rc.id}]]])}))
